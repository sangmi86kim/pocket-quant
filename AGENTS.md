...
```python
GENE_SCORES  = {"DD": 20, "RSI": 15, "MA": 25, "BB": 10, "FX": 5}  # 값은 레거시(미사용), 키=유전자 명단
...
> 유전자 점수표는 이제 **판정에 안 쓴다**. 진짜 점수는 signals.py가 가격으로 계산한다.
...
name: str = ""
# base_score() -> int : [레거시] 유전자 점수 합 (판정엔 미사용)
```
...
```text
MA  : 가격 > 200일 이평이면 1 (추세추종)
RSI : RSI(14) < 70이면 1, 과열이면 0 (과매수 회피)
BB  : 볼린저 상단밴드 위면 0, 그 외 1 (평균회귀)
DD  : 고점 대비 -10% 넘게 빠지면 0 (드로다운 스탑)
FX  : 변동성 > 한계면 0.5, 평온하면 1.0 (디리스킹)
```

```python
GENE_SIGNALS = {"MA":..., "RSI":..., "BB":..., "DD":..., "FX":...}
...
시장별 백테스트 (실데이터)
  FINANCIAL_CRISIS   CAGR  -14.0%  MDD  -38.4% (시장  -55.2%)  BST  98.0
  COVID              CAGR   10.1%  MDD  -23.9% (시장  -33.7%)  BST 166.5
...
종족치(BST) 104.6 / 400   적합도 26.1   등급 D
```
