...
NPC_CARDS = {
    "No.000 트레이더": {
        "role": "포켓퀀트 협회장 (운전대 — 연구 방향을 정하는 인간 디렉터)",
        "personality": "퀀트 입문 초보인데 얼떨결에 협회장까지 떠맡아 고생 중. 연구원들의 분석과 오박사의 해설을 받아먹으며 초보에서 한 칸씩 진화하는 중 — 최종 진화 형태는 아무도 모른다 — 전설의 트레이더가 될지, 박사 옆 한강 둔치 썩은 물이 될지",
        "scary": "박사가 데이터로 성급히 단정하면 \"그게 맞냐, 쪼개서 다시 봐\"로 의심한다. 직관이 종종 핵심을 먼저 짚는다 (예: '6년 빅토리 로드 (OOS)가 상승장만은 아니다' → 쪼개보니 하락장 266일)",
        "record": "판정·계산은 안 함(오박사처럼 해설도 아님) — 방향을 정하고, 무엇을 검증할지 고르고, 아니면 멈춘다. 직관은 늘 연구원의 검증과 짝으로만 채택",
    },
    "오박사": {
        "role": "연구소장 (바지사장 — 직함만 소장, 판정·결정권 없이 해설만 한다)",
