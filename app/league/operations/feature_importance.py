...
공통점을 본다. 장기 잣대 = 빅토리 로드 (OOS) 11년 + 사천왕 hold-out (체육관·평행세계는 합성·단기라
제외 — 우리 목표가 장기 투자라서).
...
def analyze() -> dict:
    ...
    # 장기점수 = 아레나별 백분위 평균 (빅토리 로드 (OOS)·hold-out 스케일 다르므로)
    ...
    feats.sort(key=lambda f: float(f["mean_top"]), reverse=True)
...
def _write_md(res: dict) -> None:
    ...
    lines.append(f"- 후보 {res['n']}명 중 장기(빅토리 로드 (OOS)+hold-out) 백분위 평균 상위 "
...
def main() -> None:
    res = analyze()
    print(f"후보 {res['n']}명 · 상위권 {res['k']}명 (장기 빅토리 로드 (OOS)+hold-out 백분위 평균 기준)")
    grp = ", ".join(f"{g} {c}" for g, c in sorted(res["top_groups"].items(),
                                                  key=lambda kv: -kv[1]))
