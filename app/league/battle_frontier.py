# ruff: noqa: E402
"""
battle_frontier.py - 배틀 프론티어: 챔피언 이후 엔드게임 평행세계 스트레스 테스트

시즌3부터 본편 진행은 빅토리 로드 (OOS) → 사천왕이다.
시즌4에는 난천이 챔피언 결정전으로 들어오고, 배틀 프론티어는 그 뒤에
할 일 없는 챔피언들이 더 굴러 보는 엔드게임으로 둔다.

[빅토리 로드 (OOS)와의 역할 분담]
빅토리 로드 (OOS)는 "처음 보는 데이터에서도 통하나"를 깨끗한 연도로 쟀다.
배틀 프론티어가 묻는 건 다르다: **"그 우위가 역사의 특정 '순서' 덕분(운빨)은 아닌가?"**
...
다시 붙인다. 같은 재료를 다른 순서로 섞어도 이기면 실력이다.
  ...
  - ⚠️ 한계: 훈련 기간 데이터의 재배열이므로 '새 정보' 시험이 아니다 —
    그건 빅토리 로드 (OOS)와 사천왕의 몫. 여기는 순서-운빨 검사 전담.
...
[판정 — 벤치 ≠ 사망 (상폐가 아니면 뒤진 게 아니다)]
  ...
  둘 다 = 프론티어 통과. 미달은 벤치(명단 보존, 재도전 가능).
...
def run_gate2(graduates: list) -> bool:
    ...
    npcs = [g for g in graduates if "evaluator" in g]
    # 하드 필터 통과자가 0명이면 벤치 1위가 없다 — 없이 진행 (크래시 방지)
    bench_top = next((g for g in graduates[1:]
                      if not g["specialist"] and "evaluator" not in g), None)
    entrants = [champion] + specialists
    if bench_top is not None:
        bench_top = {**bench_top, "label": f"참고(벤치1위·{bench_top['label']})"}
        entrants = [champion, bench_top] + specialists + npcs
    else:
        print("⚠️ 하드 필터 통과자 0명 — 벤치 1위 없이 관문을 진행한다.")
        entrants = entrants + npcs

    print("=== 배틀 프론티어: 챔피언 이후 엔드게임 평행세계 운빨 검사 ===")
    ...
    for g in entrants:
        ...
        for _ in range(n_worlds):
            ...
            if "evaluator" in g:
                rets, term = g["evaluator"](world, SEED_KRW)
                scores.append(0.0)
                defenses.append(False)
                bals.append(term)
            else:
                res = _score_position(
        ...
        passed = "evaluator" not in g and win_rate >= WIN_RATE_BAR and defense_rate >= DEFENSE_BAR
        if g is champion:
            all_pass = passed
    ...
    for g, arena, n, wr, dr, med, p5, passed in rows:
        if "evaluator" in g:
            mark = "🤖 NPC (참고)"
            wr_s, dr_s, med_s, p5_s = "  -  ", "  -  ", "   -  ", "   -  "
        else:
            mark = "🎫 통과" if passed else "🪑 벤치"
            wr_s, dr_s = f"{wr:>6.0%}", f"{dr:>6.0%}"
            med_s, p5_s = f"{med * 100:>+7.1f}", f"{p5 * 100:>+7.1f}"
        print(f"{g['name']:<14} {g['label']:<22} {arena:<8} {n:>4} {wr_s}"
              f" {dr_s} {med_s} {p5_s}  {mark}")
    ...
    for arena, cand_bals in bal_by_arena.items():
        ...
        for i in range(n_worlds):
            def balance_at(candidate_name: str, world_idx: int = i) -> int:
                return cand_bals[candidate_name][world_idx]

            winner = max(cand_bals, key=balance_at)
            wins_count[winner] += 1
        ...
        for candidate_name, w in sorted(wins_count.items(), key=lambda kv: -kv[1]):
            diff = cand_mean[candidate_name] - dca_mean
            sign = "+" if diff >= 0 else ""
            print(f"    {candidate_name:<14} 1등 {w:>3}/{n_worlds:<3} "
                  f"  평균 {cand_mean[candidate_name]:>10,}원 ({sign}{diff:,})")

    out = _update_regime_picks("gate2_worlds", gate2)
    print(f"\nsaved: {out}")

    print("\n본편 순서: 빅토리 로드 (OOS) → 사천왕 → 난천(시즌4 예정)")
    return all_pass
