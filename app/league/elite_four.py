...
[도전자]
시즌3 챔피언 = **NSGA-1차-t2426** (학습 stamp 20260622_195214, 비용 모델
season3_flat_1bp_band5). 균형형 14신호 가중치: FEAR_NQ·FEAR·MA·REV_RSI가
14~15%로 4축 동률 + VOL_SPIKE 11% + QQQ_SPY 10% + QQQ_DIA 8% + VOL 7% + ...
...
[비용 모델] season3_flat_1bp_band5 — 챔피언은 위탁수수료 0.1%/편도 +
슬리피지 1bp + No-trade band 5%. 성실이(DCA)는 위탁수수료 면제(토스
자동 모으기), 슬리피지 1bp는 전원 공통 부담.

실행: 프로젝트 루트에서  python -m app.league.elite_four
"""
import sys
...
# 도전자: 시즌3 종합 1위 후보 = NSGA-1차-t2426
# (학습 stamp 20260622_195214, 비용 모델 season3_flat_1bp_band5).
# 명예의 전당 v3 — reports/포켓퀀트리그/hall_of_fame_v3.md 단일 출처.
# 가중치는 SIGNAL_NAMES(=DD VOL MA MOM REV_RSI REV_BB VOL_SPIKE FEAR FEAR_NQ
# US10Y DXY SPY_TLT QQQ_SPY QQQ_DIA) 순서 raw float — combine_positions가
# 그날 의견 낸 시그널들 사이에서 비율로 다시 정규화한다.
# 시즌3 후속 본판정은 130명을 통째로 응시시키는 `app.league.v3.season3_league`
# 가 담당 — 본 단독 실행은 시즌3 챔피언 1명에 대한 봉인 해제 시뮬레이션.
CHAMPION_WEIGHTS = [
    0.07947912995673312,    # DD
    0.42747419778934503,    # VOL
    0.9106448283104521,     # MA
    0.0641730286809199,     # MOM
    0.8773290614339558,     # REV_RSI
    0.0310775800718589,     # REV_BB
    0.6769426343092638,     # VOL_SPIKE
    0.9233800938695184,     # FEAR
    0.9283253179129851,     # FEAR_NQ
    0.032983321079183114,   # US10Y
    0.1569044829200632,     # DXY
    0.003104501013030392,   # SPY_TLT
    0.6234140926996702,     # QQQ_SPY
    0.4860705883583121,     # QQQ_DIA
]
assert len(CHAMPION_WEIGHTS) == len(SIGNAL_NAMES), "챔피언 가중치 수와 SIGNAL_NAMES 불일치"
...
def _champion_returns(loaded: LoadedGym) -> pd.Series:
    """챔피언의 구간 일별 수익 (battle._score_position과 동일 공식 —
    수수료 0.1% + 슬리피지 1bp + No-trade band 5%)."""
...
def run_gate3() -> bool:
    ...
    for e in gate3:
        diff = int(e["차"])
        sign = "+" if diff >= 0 else ""
        mark = "✅" if bool(e["성실이_이김"]) else "❌"
        print(f"  {e['year']:<14} {e['regime']:<4} 챔피언 {int(e['잔고']):>10,}원  "
              f"성실이 {int(e['성실이']):>10,}원  ({sign}{diff:,})  {mark}")
...
{svg}
<p class="dim">비용 모델 <code>season3_flat_1bp_band5</code> — 챔피언은 위탁수수료 0.1% + 슬리피지 1bp + No-trade band 5% 부담, 성실이(일별 적립)는 위탁수수료 면제(토스 자동 모으기) · 슬리피지 1bp는 전원 공통.</p>
