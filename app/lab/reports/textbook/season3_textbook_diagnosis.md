...
> 세부 데이터·재현 스크립트는 `app/lab/textbook/`과 `app/lab/season/`의 각 리포트 참조.
...
## 1. 발단 — GP 후보 30명이 전부 같은 답 (`../season/v2_signal_analysis.md`)
...
```
.venv/Scripts/python.exe -m app.lab.season.v2_signal_analysis           # GP 붕괴·신호×아레나 상관
.venv/Scripts/python.exe -m app.lab.textbook.block_shuffle_explainer    # 레벨형 합성 결함 그림
.venv/Scripts/python.exe -m app.lab.textbook.synth_vs_real_regime       # 합성 vs 실제 분포
.venv/Scripts/python.exe -m app.lab.textbook.regime_distribution        # 실제 국면 분포(코덱스안 1번)
```
