...
![블록 셔플: 레벨형 vs 변화량 복원](block_shuffle_explainer.png)
...
> fix 방향(그림 아래): level형도 변화량(delta)으로 이어 연속 복원. 단 변화량만 누적하면
> 절대 레벨이 표류(평균회귀 소실, 그림 아래 8까지 흘러 clip) → **delta 연속 + 평균회귀 앵커**가 완성형.
