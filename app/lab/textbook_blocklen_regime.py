...
[출력] app/lab/outputs/textbook_blocklen_regime/ 에 png 2장 + md 리포트.
실행: .venv/Scripts/python.exe -m app.lab.textbook_blocklen_regime
"""
...
OUT_DIR = os.path.join(os.path.dirname(__file__), "outputs", "textbook_blocklen_regime")
...
def _fig_dist(real_d, results):
    ...
    for i, (g, d) in enumerate(zip(groups, series)):
        ...
        bars = ax.bar(xs + off, [d[k] for k in LABEL_ORDER], w,
