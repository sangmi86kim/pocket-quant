...
실행: 프로젝트 루트에서  python -m app.lab.diagnostics.check_signals
"""
import pandas as pd
...
def _signal(name: str) -> Callable[[pd.Series], pd.Series]:
    return cast(Callable[[pd.Series], pd.Series], SIGNAL_REGISTRY[name])
...
def run_check() -> bool:
    ...
    print("=== 1. 체육관별 노출/발동률 ===")
    header = "  ".join(f"{g:>8}" for g in SIGNAL_NAMES)
    ...
    print("\n=== 3. 판정 ===")
    corr = alldf.corr()
    event_genes = [g for g in SIGNAL_NAMES
                   if is_event_signal(_signal(g)(loaded[0].prices))]
    ok = True
    for eg in event_genes:
        others = [g for g in SIGNAL_NAMES if g not in event_genes]
