...
[출력] app/lab/outputs/regime_switching_poc/ 에 png 1장 + md 리포트.
실행: .venv/Scripts/python.exe -m app.lab.regime_switching_poc
"""
...
OUT_DIR = os.path.join(os.path.dirname(__file__), "outputs", "regime_switching_poc")
...
def _fit():
    ...
    mu = np.zeros(4); sigma = np.zeros(4)
    for s in STATES:
        r = ret_a[labels == s].dropna().to_numpy()
        mu[SIDX[s]] = r.mean(); sigma[SIDX[s]] = r.std()
...
def _fig(real_d, bb_d, rs_d, example):
    ...
    ax.set_xticks(xs); ax.set_xticklabels([REGIME_LABELS[k] for k in STATES])
    ax.set_ylabel("비율 (%)")
    ...
    ax2.set_xlabel("거래일"); ax2.set_ylabel("합성 지수")
    fig.tight_layout()
...
def _write_md(real_d, bb_d, rs_d, mu, sigma, dist_to_real):
    ...
    L.append(
        ...
        "\n재현: `.venv/Scripts/python.exe -m app.lab.regime_switching_poc`\n"
    )
