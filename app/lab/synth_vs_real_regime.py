...
[출력] 콘솔 + app/lab/reports/synth_vs_real_regime.{md,png}(그룹 막대 비교).
실행: .venv/Scripts/python.exe -m app.lab.synth_vs_real_regime
"""
...
def main() -> None:
    ...
    synth_counter = collections.Counter()
    synth_runs = []
...
def _write_md(real_d, synth_d, real_run, synth_run) -> None:
    md = ["# 합성 교과서 vs 실제 나스닥 — 국면 분포·추세 지속성 (시즌 3 교육 개혁 근거)\n",
          ...
          "\n재현: `.venv/Scripts/python.exe -m app.lab.synth_vs_real_regime`\n"]
