...
# ──────────────────────────────────────────────
# DCA 기준선 — "이길 대상"은 단순보유가 아니라 매일 적립 머신(매일 $20 자동매수).
# 금액은 비율이라 무관하다.
#
# ⚠️ 정확히는 '총자본 1/N 분할 진입'으로 적립을 **근사**한다 (코덱스 리뷰 06-11):
#   진짜 적립은 매일 새 돈이 들어오는 외부 현금흐름이라 성과를 IRR(돈이 들어온
#   시점을 반영한 수익률)로 재야 하는데, 전략 쪽은 첫날부터 총자본을 들고
#   시작하므로 그대로 비교하면 자(尺)가 안 맞는다. 양쪽을 같은 자본 기준
#   (첫날 총자본 보유)에 놓아야 score_vs_dca의 "총수익 빼기" 비교가 성립한다.
#   1차 근사에서 둘은 같고 복리 교차항에서만 갈린다 — 기준선 용도로 충분.
# ──────────────────────────────────────────────
def _dca_position(loaded: LoadedGym):
    """매일 적립(DCA)의 분할 진입 근사를 포지션 스케줄로 표현한다.

    평가 구간이 N거래일이면 매일 종가에 총자본의 1/N씩 사 모은다:
    k번째 거래일 매수 후 투자 비중 = k/N (시작 0 → 끝 100%, 매도 없음).
    같은 채점기(_score_position)를 타므로 하루 lag도 전략과 동일 적용.
    수수료는 fight_dca가 0으로 둔다 (자동 적립 매수 수수료 면제 — 그쪽 참고)."""
