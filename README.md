...
|------|------|------|
| 🏟️ 체육관 (훈련) | QQQ 실데이터 6국면 백테스트 | ✅ 운영 중 (스타팅 6마리 시그널, 가중치 6차원) |
...
- `python tools/e2e.py` — 🚀 전 파이프라인 스모크 (10초 안에 게이트 4 + 진단 2 + walk_forward + NSGA-III)
...
- `python tools/walk_forward.py` — 🎫 선발 과정 OOS (파라미터로 자산/훈련길이/비용 민감도)
