...
| 잠만보 | '전부 현금' 전략 — 길막고 아무것도 안 함 (최적화의 숙적). 반대 극단은 **B&H(항상 풀매수)** = 그냥 사서 계속 들고 있기 — 둘 다 기준선으로 시험에 같이 출전한다 |
...
| 체육관 | 기간 | 성격 |
|--------|------|------|
| 🕸️ 2000-02 닷컴 붕괴 | 2000-03 ~ 2002-12 | 갈아내리는 긴 하락(-83%) → 역발상엔 칼날 잡기 지옥, 드로다운 스탑 천국 |
| 💀 2008 금융위기 | 2008-01 ~ 2009-06 | 시스템 붕괴 대폭락 → 방어형 천국, 공격형 지옥 |
| 🌅 2009-10 회복장 | 2009-03 ~ 2010-12 | 바닥 후 재진입(+101%) → 겁먹고 현금 들고 있으면 반등을 통째로 놓침 |
| 🦠 2020 코로나 급락 | 2020-02 ~ 2020-06 | V자 급락 후 즉시 회복 → 버티면 생존 |
| 🔥 2017 불타입 상승장 | 2017-01 ~ 2017-12 | 저변동성 꾸준한 우상향 → 추세추종/모멘텀 강세 |
| 🌫️ 2015-16 횡보장 | 2015-01 ~ 2016-12 | 방향 없이 출렁임 → 추세형 헛신호, 평균회귀 유리 |

> 💡 하락 3 / 회복·상승·횡보 3으로 일부러 균형을 맞췄다 (회복장은 "시험지가 하락
> 편향"이라는 지적으로 2026-06-11 추가 — 트레이너의 철학: **나스닥 장기 우상향 베팅,
...
평균만 좋고 한 체육관에서 처참한 트레이너는 챔피언이 될 수 없다.
등급: `S`(≥90) `A`(≥70) `B`(≥50) `C`(≥30) `D`(그 외). 종족치 `BST = 4스탯 합` (표시용).

### 🧟 잠만보 봉인의 역사 — "아무것도 안 하기"는 세 번 부활했다

최적화기는 목적함수의 빈틈을 반드시 찾아낸다. '전부 현금'(잠만보)이 1등을 먹으려던 경로 3개를 전부 실측으로 잡아 봉인했다:

| # | 부활 경로 | 봉인 |
|---|----------|------|
| 1 | HP(현금)를 적합도에 포함 → 잠만보 69점 1위 | HP 가중치 0 + DEF=Calmar 재설계 |
| 2 | 최약 체육관 가중 50% → 잠만보는 "최약 과목이 없어서" 30위까지 부상 | min 가중 30% (게이트 지키는 실측 최대치) |
| 3 | DCA 대비 점수를 평균으로 결합 → 하락장에서 잠만보가 라이벌을 이김 | 평균 결합 금지, NSGA-III에서 벡터 그대로 |

> 감시 장치: `python tests/test_baselines.py` — 잠만보가 하위 25% 밖으로 기어나오면 FAIL.
...
> 🪙 단, 관문은 사형장이 아니다 — **"상폐가 아니면 뒤진 게 아니다"** (트레이너 슬로건).
...
- `python tests/test_baselines.py` — 🧟 잠만보 감시 (퇴화 게이트)
...
- ✅ **v0.3.1~2 적합도 퇴화 수정 + 거래비용** — 잠만보 1차 봉인, 수수료 0.1%
...
- 🏫 **아카데미 (팔데아 지방 개척)** — **나스닥 데이터로 cGAN을 학습시켜 "다른 지방"의 시장을 합성**하는 생성기. 부트스트랩(기존 역사 재배열)을 넘어, 국면 라벨을 조건(condition)으로 나스닥의 통계적 질감을 가진 **새로운 가격 경로**를 생성 → 새 지방의 체육관을 개설해 트레이더를 유학 보낸다. 본 적 없는 지방에서도 통하는 트레이더 = 진짜 robust. (학습 재료도 hold-out 봉인 규칙 적용 — 2020-07 이후 데이터로는 학습하지 않는다)
