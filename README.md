...
> 다목적 챔피언은 `[VOL·REV_RSI·REV_BB]` 동일가중이었음. v1 마감 결과는 [`hall_of_fame_v1.md`](reports/포켓퀀트리그/hall_of_fame_v1.md).
...
> (다음 시즌은 `v2/` 신설로 시작). 관문 코어는 graduates를 외부 주입받으므로 단독 실행 불가.

#### 시즌 v1 결과 (2026-06-13 마감)

리그 v1 시즌 챔피언 = **TPE-s11** (단일목적 누적 잔고 max, REV_RSI 55% + REV_BB 45%).

- 다목적 NSGA-III 라인업(TOP10) + 단일목적 TPE 5시드 + 옛 동일가중 챔피언 동시 출전
- **QQQ 평행세계 ② 토탈 본판정**: TPE 5명이 1~5위 (단일목적 압승, 다목적 TOP06 6위로 밀림)
- **OOS 11년 ①**: TPE 5명 전원 게이트 통과 (다목적은 TOP03만)
- **자산-횡단 검증 (SPY)**: 모든 관문에서 어플삭제맨(B&H) 1위 → **TPE-s11은 QQQ 한정 챔피언**

자세한 결과는 **[`reports/포켓퀀트리그/hall_of_fame.md`](reports/포켓퀀트리그/hall_of_fame.md)** (시즌별 누적 기록).

> 📌 **"튜닝의 끝은 순정"** — 6마리 시그널 중 REV 2마리만 살아남고 DD/VOL/MA/MOM 4마리는
> 가중치 0%로 떨어졌다. 다목적 NSGA-III가 풀(VOL 포함 분산)로 깎아낸 답보다, 단일목적이
> 직접 잔고 합 max로 짜낸 가중치(REV 둘만)가 더 정직한 챔피언이었다.

> 🌲 **자산-횡단 한계** — TPE-s11이 SPY 가면 어플삭제맨에게 모든 관문에서 짐. 현 시그널 풀
> (가격 6마리)은 QQQ 닷컴/리먼 패턴에 학습돼 있어 자산 바뀌면 무력. **진짜 알파는 가격 외
> 정보원이 필요** — 시즌 v1.x = 야생 잡기 (자세한 화두는 [GitHub Issues](../../issues)).

#### 시즌 v1.x 진행 상황 (2026-06-13 자정 기준)

- ✅ **야생 7마리 잡음** (위 표) — 풀 6 → 13마리. 룩어헤드 검사 13/13 PASS.
- ✅ **CMA-ES 엔진 신설** — NSGA-III(다목적) · TPE(단일목적 Bayesian) · CMA-ES(단일목적 진화전략) 3종 옵티마이저.
- ⏳ **본탐색 + 챔피언로드** — 13마리 풀로 5시드 sweep + SPY 자산-횡단 검증 (대기).
...
| 단계 | 무엇 | v1 상태 (2026-06-13 마감) |
|------|------|------|
| 🏟️ 체육관 졸업시험 | QQQ 실데이터 6국면 백테스트 | ✅ 운영 중 (13마리 시그널, 실데이터 검증장) |
| 🎫 리그 본선 (OOS 11년) | 안 본 평시 연도에 출전 | ✅ TPE 5명 전원 통과 (다목적은 TOP03만) |
| 🗼 배틀 프론티어 (평행세계 400) | 21일 블록 부트스트랩 — "있을 법했지만 없었던 역사" 본판정 | ✅ **TPE-s11 1위 5.01억 (+25%)** — 다목적 TOP들 6위 이하 |
| 👑 사천왕 (hold-out 7라운드) | 2020-07~ 봉인 미래 — 06-11 1회 사용으로 소진 | 🔒 봉인 소진 (강세장 6년이라 차별점 없음) |
| 🌲 자산-횡단 (SPY 리그) | 같은 가중치 챔피언이 SPY에서도 통하는지 | ❌ 어플삭제맨이 OOS·사천왕 모두 1위 — **챔피언은 QQQ 한정** |
...
- ✅ **아카데미 v2 (블록 부트스트랩) + 리그 2차** — 합성 평행세계로 훈련 → 실QQQ 졸업시험 → 리그(관문 ①②). 누수·졸업게이트·HV(비표준 1.0캡 → optuna 정확·무캡) 3중 수술 후 첫 정상 시즌. **공정한(랜덤 진입) buy-hold 대비 수익 알파 입증** — 옛 day-1 B&H는 체육관 첫날 완벽진입 특혜(+7.4%)였음(어플삭제맨 5명 랜덤진입으로 공정화). GP는 한 답 복붙으로 벤치. 시즌 2 챔피언 = NSGA-t5938(크로스에셋 성장 틸트). 자세히: [`hall_of_fame_v2.md`](reports/포켓퀀트리그/hall_of_fame_v2.md)
- 🏫 **아카데미 v1 (cGAN, 팔데아 지방 개척)** — **나스닥 데이터로 cGAN을 학습시켜 "다른 지방"의 시장을 합성**하는 생성기. 부트스트랩(기존 역사 재배열)을 넘어, 국면 라벨을 조건(condition)으로 나스닥의 통계적 질감을 가진 **새로운 가격 경로**를 생성 → 새 지방의 체육관을 개설해 트레이더를 유학 보낸다. 본 적 없는 지방에서도 통하는 트레이더 = 진짜 robust.
