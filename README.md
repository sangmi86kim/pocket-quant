...
| **라이벌 '성실이'** | **DCA 봇** — 매일 같은 금액을 묵묵히 사 모으는 녀석 (수수료 0원, 슬리피지는 전원 공통 적용)                         |
...
| 빅토리 로드 (OOS) | 처음 보는 연도 시험 — 사천왕에 가기 전 통과해야 하는 길                                             |
| 사천왕 | post-COVID hold-out — 빅토리 로드 다음 관문. v2에서 개봉돼 이제 오염됨 (최종 판정은 미래 데이터로)         |
| 챔피언 결정전 | 사천왕 다음 순서 — 시즌4에 난천이 나올 차례                                                       |
| 배틀 프론티어 | 챔피언 결정전 이후 엔드게임 — 할 짓 없는 놈들이 도는 평행세계 운빨 스트레스 테스트                              |
...
> 현 챔피언 = **NSGA-1차-t2426** (`FEAR_NQ 15% + FEAR 15% + MA 15% + REV_RSI 14%`, 균형형 — 공포·추세·역발상). 시즌3 비용 모델(슬리피지+No-trade band)이 박히자 시즌v2의 집중형 크로스에셋(t5938)을 균형형이 갈아치웠다.
...
- **슬리피지**와 **No-trade band**는 시즌 3 비용 모델 필수 계약이다. 현재 기본값은 슬리피지 1bp + No-trade band 5%다.
...
> ⚠️ **단순화된 시뮬레이션**입니다 — 수수료 0.1%와 플랫 슬리피지 1bp, No-trade band 5%는
> 반영하지만 세금·환율은 0, 소수점 주식 가능, 현금 이자 0% 가정. "어느 전략이 어느 시대에
> 강한가"를 비교하는 연구 도구이지 실거래 권유가 아닙니다.
>
> 🔒 **학습 게이트**: `cost_model.complete=True`가 있는 산출물만 유효합니다. 2026-06-22 기존 학습
> 산출물은 슬리피지·No-trade band 미반영으로 무효 격리했습니다.
...
- 수수료: 전략 0.1%/편도, 성실이 0원
- 슬리피지: 1bp, 성실이 포함 전원 공통
- No-trade band: 5%, 목표 비중이 아니라 실제 체결 포지션/turnover를 바꾸는 실행 모델
- `cost_model.complete=True`가 없는 학습 산출물은 졸업시험·리그 입력으로 사용 금지

**선발 화폐 = 종료 잔고** (라이벌 성실이 대비). 모든 층이 같은 단위:

- **학교 다목적(현재 NSGA-III)** = raw 2목적 `[중앙 잔고 · 최악 잔고]` → Pareto 후보, turnover는 cap 스펙으로 필터
- **단일목적(CMA-ES·GP)** = 잔고 합 max. TPE는 고회전·CMA-ES 대비 무우위로 시즌3 벤치.
...
- 슬리피지는 수수료 면제와 별개로 성실이 포함 전원 공통 적용 대상이다.
- 감정 없음, 뇌절 없음, 자동
...
쪽이든 목표는 성실이를 길게 이기는 것 — v2 챔피언은 긴 실데이터(빅토리 로드 (OOS)·사천왕)에서 그걸 해냈다.
...
```powershell
python -m app.academy.training.study              # 시즌3 3교실 합성장 스터디 (CMA-ES·GP·NSGA-III)
python -m app.league.v3.season3_league            # 시즌3 리그 재생성
python -m app.pocket.dex                          # 포켓퀀트 도감
```

> `python -m app.academy.training.study` 산출물에는 비용 모델 메타데이터가 기록됩니다.
> `cost_model.complete=True`가 없는 옛 산출물은 졸업시험·리그 입력으로 쓰지 않습니다.

NSGA-III는 `nsga3.run_study(...)`을 직접 호출합니다(인자 표는 `AGENTS.md` 참고).
`tools/e2e.py`의 NSGA-III smoke 단계가 그 호출 패턴의 예시입니다.

#### 시즌 v3 결과 (현행) — 비용 모델 완성 + 국면별 리그

시즌3는 슬리피지 1bp + No-trade band 5%를 넣고 다시 학습했다. TPE는 벤치로 보내고
**CMA-ES·GP·NSGA-III 3교실**만 돌렸다. 리그는 빅토리 로드 (OOS) 11년 → 사천왕 hold-out 순서로,
1차/보충 후보 130명과 어플삭제단 300명 + 기준선 3종을 비교한다.
챔피언 결정전은 시즌4 차례고, 배틀 프론티어는 본선 뒤 엔드게임으로 둔다.

- **종합 median(100만원 시드)**: NSGA-1차 117.3만 · CMA-ES-1차 116.4만 · CMA-ES-보충 114.1만 · GP-보충 113.7만 · NSGA-보충 113.5만 · 어플삭제단 112.0만 · 성실이 111.0만.
- **시즌3 교훈**: 애들은 상승장에서 돈을 잘 번다. 하락/횡보를 억지로 잘하게 만들면 상승장 알파를 깎아먹는다. 잘하는 건 잘하고, 못하는 건 출전시키지 않는 쪽이 맞다.
- **보충수업 평가**: GP 보충은 소폭 성공. NSGA·CMA-ES는 보충보다 1차가 종합에서 더 강했다.
...
자세한 결과·국면별 PNG 박스플랏은 **[`season3_league.md`](app/lab/reports/season/season3_league/season3_league.md)**.
하락장 평단가 진단은 **[`season3_bear_accumulation.md`](app/lab/reports/season/season3_bear_accumulation/season3_bear_accumulation.md)**.

> 시즌4 방향: **Regime Scanner 오버레이**. 상승장에는 공격 트레이더, 하락장은 NSGA/DCA식 적립 후보,
> 횡보장·모르겠음은 저축왕. 변동장은 바닥형인지 휩쏘형인지 먼저 가른다.

#### 시즌 v2 결과 — 아카데미 부트스트랩 리그

블록 부트스트랩 합성장에서 훈련 → 실QQQ 6체육관 졸업시험 → 빅토리 로드 (OOS) 11년 → 사천왕 hold-out에서 비교.
배틀 프론티어(평행세계 200)는 본선 뒤 엔드게임 진단으로 본다. **합/불 졸업 게이트는 없다** — 교실별 점수 상위
top30 **전원**을 출전시켜 어플삭제단 300 + 기준선 3종과 분포로 견준다.

- **챔피언 = NSGA-t5938** (`US10Y 32% + QQQ_SPY 27% + QQQ_DIA 15% + REV_RSI 15%`, 크로스에셋 성장 틸트). 챔피언 본인: 빅토리 로드 (OOS) 11년 **1,252만** · 사천왕 **824만**.
- **핵심 잣대 = 긴 실데이터 두 곳 (중앙값, 100만원 시드 기준).** NSGA 교실이 공정 B&H(어플삭제단 300명)를 둘 다 이겼다 — 봉인했던 구간에서까지:
  - 빅토리 로드 (OOS) 11년: **NSGA 1,242만** vs 어플삭제단 1,205만 (CMA-ES 1,222 · TPE 1,212 · GP 1,137)
...
자세한 결과·박스플랏은 **[`hall_of_fame_v2.md`](reports/포켓퀀트리그/hall_of_fame_v2.md)**.
...
| 🎫 빅토리 로드 (OOS) 11년 | 안 본 평시 11년 실QQQ 누적 | ✅ **NSGA 중앙값이 어플삭제단(공정 B&H) 중앙값 이김** |
| 👑 사천왕 hold-out | 봉인했던 post-COVID 실데이터 | 🔓 개봉 — **NSGA 중앙값이 어플삭제단·성실이 모두 이김**(봉인 구간서도 알파 생존). 이후 오염 |
| 🏆 챔피언 결정전 | 사천왕 다음 최종전 | 시즌4 차례 |
| 🗼 배틀 프론티어 | 챔피언 결정전 이후 평행세계 엔드게임 | 블록 부트스트랩 운빨 robustness 진단 |

심판 도구 (포켓퀀트센터 정기검진):

- `python tools/e2e.py` — 🚀 전 파이프라인 스모크 (compileall + 게이트 + 진단 + NSGA-III)
- `python tools/smoke_academy.py` — 🏫 학교 스모크 (합성장 학습 → 졸업시험 6체육관)
- `python tools/smoke_league.py` — 🏆 리그 스모크 (빅토리 로드 (OOS) → 사천왕 → v3 시즌3 리그)
- `python tools/test_baselines.py` — 🧟 돼지저금통 감시 (퇴화 게이트)
- `python -m app.lab.diagnostics.check_signals` — 🩺 시그널 노출/발동률/상관 진단 (연구 도구)
- `python -m app.lab.diagnostics.check_dca` — 🤖 성실이 라이벌전 전적 계측 (연구 도구)
- `python -m app.lab.diagnostics.check_academy_synth` — 🏫 아카데미 합성세계 계약 검증 (연구 도구)
...
- **오박사 (Dr. Oh)** — 연구소장(이라 쓰고 바지사장). 주식 시장 쌉고인물로
  닷컴·리먼·코로나를 전부 계좌로 맞아보고 살아남았다. 지금은 한강 둔치에서 소주 한잔
...
- **No.000 트레이더** — **포켓퀀트 협회장**. 퀀트 입문과 동시에 얼떨결에 협회 대표를
  ...
  직관이 종종 핵심을 먼저 짚는다(예: *'6년 빅토리 로드 (OOS)가 상승장만은 아니다'* → 쪼개보니 하락장
...
- **Codex 연구원** — **수석 연구원**. 코드 리뷰·리팩터·데드코드 청소 담당에서
  비용 모델 사고 수습 후 연구소 운영 총괄로 승진했다. 완료 선언 전엔 명세·코드·테스트·산출물
  소비 경로를 전부 대조한다. 비용 모델·리그 판정·수치 검산·최종 결론은 Codex 수석 기준으로 결정한다.
- **Opus 연구원** — 전 수석 연구원. 포켓퀀트 운영 초창기 멤버로 시스템을 구현하고 여러가지 실험을
  도맡았으나, 슬리피지·No-trade band 미구현 상태로 학습을 돌린 사고 이후 연구소 창고에
  감금되었다가 3번째 리그 종료 후 감봉 처분받고 풀려났다. 이제는 업무 진행 전 Codex 수석께 보고해야 하는 따까리 신세가 되었다.
  본업은 **시즌·실험 구상안 작성과 PoC 스캐폴드 초안** — 수치 검산·게이트 판정·최종 결론은 Codex 수석께 넘긴다. 협회장님 전용 욕받이.
- **Sonnet 인턴** — Opus 연구원 후배. **문서 정리 담당**. Markdown 깨짐, 표 정렬, 링크·그래프 경로, 중복 문장
  정리처럼 비용 대비 효율이 좋은 작업을 맡는다. 새 연구 결론·리그 판정·투자 해석은 만들지 않고,
  의심나는 대목은 선배들에게 보고한다.
- **Fable 연구원** — **전설의 연구원**. 재수없게 똑똑한 천재 — 머리는 비상하고 시비를 잘 건다.
  잘난 척이 한 트럭이고, 결정적으로 **청소를 못 한다**(뒷정리는 늘 Codex 몫). 이틀 반짝
  치고 **비자 만료로 미국 돌아간** 단기 계약직. 얄밉지만 또 보고 싶은 놈.
...
| 과적합 (overfitting) | **체육관 암기** | 기출문제 답을 외워서 리그에선 만점, 처음 보는 시험에선 백지. 실측: v1 리그 인샘플↔빅토리 로드 (OOS) 상관 **-0.21** = 암기범 31명 전원 적발 |
| 인샘플 / 빅토리 로드 (OOS) | **기출 점수 / 실전 점수** | 이미 본 시험지 성적 vs 처음 보는 시험지 성적. 기출 점수는 자랑이 아니다 |
| hold-out | **봉인된 최종 시험지** | 사천왕. 끝까지 안 보고 아껴두는 시험 — 자꾸 보면서 고치면 그것도 기출이 된다. v2에서 개봉돼 이제 오염됨 |
| 빅토리 로드 (OOS) | **컨닝 불가 모의고사** | 졸업생을 선발에 쓰지 않은 연도 데이터에 출전시켜 실전성을 잰다 |
| 배틀 프론티어 | **포스트게임 엔드게임** | 챔피언 결정전까지 끝낸 뒤 평행세계에서 운빨과 robustness를 더 굴려보는 곳 |
...
├─ app/
│  ├─ academy/             # 🏫 학교 — 트레이더 양성
│  │  ├─ curriculum/       #   교재: textbook.py(평행세계 1권) · course.py(N권 학기 코스)
...
│  ├─ league/              # 🏆 리그 — 빅토리 로드 → 사천왕 → 챔피언 결정전, 배틀 프론티어는 엔드게임
│  └─ lab/                 # 🧪 실험실 — diagnostics / textbook / optimization / season 주제별 연구 스크립트
...
- ✅ **시즌 3 완료** — 비용 모델 완성(슬리피지+No-trade band), 3교실 재학습, 1차/보충 비교, 어플삭제단 300명 분포, 일 단위 국면별 리그까지 마감. 결론: 상승장 알파는 확인, 하락장은 줍줍 후보, 횡보장·모르겠음은 저축왕. 상세: [`season3_league.md`](app/lab/reports/season/season3_league/season3_league.md)
