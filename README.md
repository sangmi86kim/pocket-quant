...
| **라이벌** | **DCA 봇** — 매일 같은 금액을 묵묵히 사 모으는 녀석 (수수료도 0원) |
| 잠만보 | '전부 현금' 전략 — 길막고 아무것도 안 함 (최적화의 숙적) |
...
그리고 이 게임의 진짜 최종 보스는 시장이 아니라 **라이벌(DCA 봇)** 이다 —
...
이 게임의 라이벌은 화려한 기술이 없다. **매 거래일 같은 금액으로 QQQ를 사 모을 뿐이다.**
그런데 무섭다:
...
`battle.fight_dca()`가 라이벌을 소환하고, `score_vs_dca`로 라이벌전 점수를 매긴다:

```
score_vs_dca = 0.4×(수익 차이) + 0.4×(낙폭 개선) + 0.2×(샤프 차이)    # 양수 = 라이벌보다 강함
```

실측: **여섯 체육관 전부에서 라이벌을 이기는 조합은 0개.** 위기 체육관을 이기면
...
| 🗼 배틀 프론티어 (합성 스트레스) | 블록 부트스트랩으로 "있을 법했지만 없었던 역사" ~100개 세계에서 생존률 | ⏭️ NSGA-III 후 |
