...
python app/league/victory_road.py                 # 챔피언로드 관문 ① 코어 (OOS 11년)
python app/league/v1/champion_road_lineup.py      # v1 시즌 풀라인업 어댑터
...
> **2026-06-14 리그 정리**: 시즌제 폴더로 분리. 관문 코어(victory_road · battle_frontier ·
> elite_four)는 root, 시즌 어댑터는 `v1/`·`v1x/`, 시즌 무관 운영(탐색·후처리)은 `operations/`.
...
│  │  └─ training/         #   학습법: tpe · cma_es · gp(단일목적) · nsga3(다목적) · _single_obj(공통)
│  ├─ pocket/              # 🎒 포켓퀀트 — signals.py(시그널 13마리) · battle.py(백테스트) · models.py · dex.py(도감) · eggs/
│  ├─ world/               # 🌍 시장 — data.py(yfinance + 캐시) · regime.py(국면 라벨)
│  ├─ league/              # 🏆 리그 — victory_road(챔피언로드 ①) · battle_frontier(②) · elite_four(③) · sweep_seeds
