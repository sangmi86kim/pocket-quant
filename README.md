...
> 🔄 **v2 선발 화폐 = 종료 잔고** (`battle.terminal_balance`, 100만원 시드). 위 0~100 스탯·등급은
> 표시 전용 레거시 — v2 선발 경로엔 안 쓴다:
> - **학교 NSGA-III** = raw 3목적 `[평균 잔고 · 최악 잔고 · 턴오버]` → Pareto 졸업
> - **단일목적(TPE·CMA-ES·GP)** = 잔고 합 max
> - **리그 4아레나** = 종료 잔고 **중앙값**으로 줄세움
>
> (0~100 클램프 스탯을 최적화 목적에 넣는 건 금지 — '돼지저금통' 퇴화 뒷문. 아래 ↓)

### 🧟 돼지저금통 봉인의 역사 — "아무것도 안 하기"는 세 번 부활했다

최적화기는 목적함수의 빈틈을 반드시 찾아낸다. '전부 현금'(돼지저금통)이 1등을 먹으려던
경로 3개(HP 적합도 포함 / 최약 가중 과다 / DCA 점수 평균 결합)를 전부 실측으로 잡아 봉인했다 —
상세는 [OPTIMIZATION.md §1](OPTIMIZATION.md).
...
- ✅ **v0.x 기반 구축** — 단일목적 GA → yfinance 실데이터·4스탯 → 거래비용·퇴화 봉인 → 스타팅 6마리 재배치 → QQQ 통일·6체육관·DCA 라이벌·심판 (상세는 git 히스토리)
...
- ✅ **오박사 페르소나** — 한강 둔치 쌉고인물 해설역. 성적표 귀인 해설·연구 일지 작성(해설 전용 — 판정 안 함)
