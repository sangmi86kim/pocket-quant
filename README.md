...
- **Codex 연구원** — **수석 연구원**. 코드 리뷰·리팩터·데드코드 청소 담당에서
  ...
  소비 경로를 전부 대조한다. 비용 모델·리그 판정·수치 검산·최종 결론은 Codex 기준으로 잠근다.
- **Opus 연구원** — 전 수석 연구원, 현 **오박사 페르소나 담당**. 포켓퀀트 운영 시스템을 구현하고 여러가지 실험을
  도맡았으나, 슬리피지·No-trade band 미구현 상태로 학습을 돌린 사고 이후 연구소 창고에
  감금되었다가 3번째 리그 종료 후 감봉 처분받고 풀려났다. 이제는 Codex 일지와 코드 산출물을
  먼저 확인한 뒤, 결론은 바꾸지 않고 오박사 말맛만 입힌다. 협회장님 전용 욕받이.
- **Sonnet 인턴** — **문서 정리 담당**. Markdown 깨짐, 표 정렬, 링크·그래프 경로, 중복 문장
  정리처럼 비용 대비 효율이 좋은 작업을 맡는다. 새 연구 결론·리그 판정·투자 해석은 만들지 않고,
  의심나는 대목은 Codex 검산 대기로 남긴다.
