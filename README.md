...
그래서 찾는 건 영웅 한 마리가 아니라 **어떤 국면이 와도 안 무너지는 전략 집합**이다.
잣대는 화려한 백테스트가 아니라 **라이벌 '성실이'(DCA 봇) — 매일 수수료 0원으로 묵묵히
사 모으는 녀석 — 을 길게 이기는가.** 좋을 때만 반짝하고 위기에 녹는 전략은 데려갈 이유가 없다.

> 🧭 **이건 알고리즘 연구가 아니다.** 유전 알고리즘이든 NSGA-III든 RL이든 — 돈을 더 잘,
> 더 안정적으로 버는 도구면 뭐든 갈아끼운다. 특정 알고리즘·전략에 갇히면 큰 흐름을 놓친다.
> (지금 챔피언을 뽑는 도구는 NSGA-III일 뿐. 탐색 정식화: [OPTIMIZATION.md](OPTIMIZATION.md))
...
낸다(robustness ↔ 수익 트레이드오프). 이 지도(Pareto front)를 그리는 게 다목적 최적화의 일이고,
v2 챔피언은 긴 실데이터(OOS·사천왕) 중앙값에서 성실이를 넘었다.
