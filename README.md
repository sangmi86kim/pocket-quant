...
| **라이벌 '성실이'** | **DCA 봇** — 매일 같은 금액을 묵묵히 사 모으는 녀석 (수수료 0원, 슬리피지는 전원 공통 적용 예정)                      |
...
- **슬리피지**와 **No-trade band**는 시즌 3 비용 모델 필수 계약이다. 현재 legacy 엔진은 둘 다 미반영이라 긴 학습이 잠겨 있다.
...
> ⚠️ **현재 legacy 시뮬레이션**입니다 — 수수료 0.1%는 반영하지만 세금·슬리피지·환율은 0,
> 매일 정확히 비중 조정, 소수점 주식 가능, 현금 이자 0% 가정. 이 상태의 학습 결과는
> 최종 판정용으로 쓰지 않습니다. "어느 전략이 어느 시대에
> 강한가"를 비교하는 연구 도구이지 실거래 권유가 아닙니다.
>
> 🔒 **학습 게이트**: 시즌 3 학습은 ① **슬리피지**(체결가 밀림) — 성실이(DCA) 포함 전원 공통,
> 수수료만 비대칭 ② **노트레이드밴드** — 매일 정확히 맞추지 않고 비중이 어지간히 벗어날 때만
> 거래(불필요한 잦은 매매 억제)를 구현한 뒤에만 유효합니다. 둘은 한 세트(슬리피지만 넣으면
> 가짜 매매에 과징수). 2026-06-22 학습 산출물은 비용 모델 미완으로 무효 격리했습니다.
...
- 슬리피지: 성실이 포함 전원 공통 적용 예정
- No-trade band: 목표 비중이 아니라 실제 체결 포지션/turnover를 바꾸는 실행 모델로 구현 예정
...
> 현재 `python -m app.academy.training.study`는 비용 모델이 `commission_only_legacy`라 즉시 중단됩니다.
> 슬리피지 + No-trade band 구현 후 `cost_model.complete=True`가 된 산출물만 유효합니다.
