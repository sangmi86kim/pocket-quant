...
[데이터 범위 — hold-out 봉인]
기본 실행은 봉인 직전(2020-06-30)까지만 쓴다. 2020-07 이후 관찰은
include_holdout=True 로만 열리며 관찰 전용이다 — 그 결과로 선발 로직을
고치면 오염 (AGENTS.md 5·6번).
...
# ── 기본 설정 ──────────────────────────────────────
# hold-out 봉인 (AGENTS.md 5·6번): 2020-07 이후는 선발·적합도 계산에 못 들어간다.
# 기본 실행은 봉인 직전까지만 본다 — 예전 기본값(2026-06-09)은 2021년 이후 출전
# 선발의 훈련 구간에 봉인 데이터가 들어가는 누수였다 (코덱스 리뷰 P1, 06-11).
DATA_END = "2020-06-30"
# include_holdout=True 전용 데이터 끝. 결과는 '미래 구간 관찰'일 뿐 —
# 보고서를 보고 조합·파라미터·가중치를 바꾸는 순간 오염이다.
HOLDOUT_DATA_END = "2026-06-09"
...
def run_walk_forward(ticker: str = "SPY", train_years: int = 4,
    ...
    include_holdout: True면 봉인 구간(2020-07~)까지 데이터를 연장한다 — 관찰 전용.
    이 결과로 조합·파라미터·가중치를 바꾸면 hold-out 오염이다 (AGENTS.md 5·6번).
    반환: 요약 dict (민감도 스윕에서 표로 모으는 용도)."""
    cost_backup = battle.TRADE_COST
    if trade_cost is not None:
        battle.TRADE_COST = trade_cost
    try:
        return _run(ticker, train_years, verbose, first_test_year, include_holdout)
    finally:
        battle.TRADE_COST = cost_backup


def _run(ticker: str, train_years: int, verbose: bool,
         first_test_year: int | None, include_holdout: bool = False) -> dict:
    data_end = HOLDOUT_DATA_END if include_holdout else DATA_END
    if include_holdout:
        print("⚠️  hold-out(2020-07~) 포함 모드 — 결과는 관찰 전용이다.\n"
              "    이걸 보고 조합/파라미터/가중치를 손대면 봉인 오염 (AGENTS.md 5·6번).")
