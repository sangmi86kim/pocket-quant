...
[골든 넘버의 출처]
2026-06-22 시즌3 엔진(전 체육관 QQQ 통일 · 적합도 70/30 · 전략 수수료 0.1%
+ 슬리피지 1bp + No-trade band 5% · DCA 기준선 수수료 0원/슬리피지 1bp)에서
풀 정밀도로 실측한 값. 캐시된 가격(data_cache/)을
쓰므로 오프라인에서도 돈다.
...
GOLDEN_FITNESS = {
    "VOL+REV_RSI+REV_BB": 42.428662982453574,              # 현 챔피언
    "VOL+REV_RSI": 41.45958882883845,                   # 2위
    "DD+VOL+MA+MOM+REV_RSI+REV_BB": 29.903355220610234,  # 전 유전자 합체
}

# 챔피언의 닷컴 체육관 raw 지표 (총수익 / 최대낙폭 / 샤프 / 일평균 턴오버)
GOLDEN_CHAMPION = "VOL+REV_RSI+REV_BB"
GOLDEN_CHAMPION_DOTCOM = {
    "total_return": 0.1124491804177874,
    "max_drawdown": -0.12985199994651875,
    "sharpe": 0.33134224675306634,
    "turnover": 0.06882022471910113,
}

# DCA 기준선 (수수료 0원, 슬리피지 1bp) raw 지표
GOLDEN_DCA = {
    "닷컴 붕괴 체육관": {
        "total_return": -0.43453980363538014,
        "max_drawdown": -0.5347867449606036,
        "sharpe": -0.6577329729970008,
    },
    "회복장 체육관": {
        "total_return": 0.2888070601663053,
        "max_drawdown": -0.1063468182935422,
        "sharpe": 1.3652111289571598,
    },
    "코로나 급락 체육관": {
        "total_return": 0.15682956980699414,
        "max_drawdown": -0.06586927820292976,
        "sharpe": 1.9856656761222518,
    },
}

GOLDEN_CHAMPION_SCORE_VS_DCA_AVG = 0.15158175567798973
...
def run_check() -> bool:
    ...
    print("\n=== 3. DCA 기준선 raw 지표 (수수료 0원, 슬리피지 1bp) ===")
