...
시즌3 리그는 두 관문만 본다: ① OOS(victory_road) → ② 사천왕(elite_four).
...
SEED_KRW = 1_000_000
OOS_SMOKE_YEARS = 2     # OOS 앞 몇 개 연도만 (스모크라 소수)
...
def main() -> int:
    ...
    print("관문: OOS(victory_road) → 사천왕(elite_four) · v3 130명 본경기\n")
    weights = [1.0] * len(SIGNAL_NAMES)   # 졸업생 대역 — 동일가중 1명
    rows = []
    try:
        prices = get_prices(VR.TICKER, "1999-03-10", "2026-06-09")
        rows.append(("OOS", _check_oos(prices, weights)))
